...
A **portable skill package** for building and operating a **Living Strategy Agent** (canonical: `.claude/skills/living-strategy-agent/SKILL.md`). This repo also ships **`google_trends_report`** (`.claude/skills/google_trends_report/`) for URL-based trend intelligence reports. The **`openclaw/`** folder holds paste-ready copies of both skills.

## Claude Code plugin

**`plugins/living-strategy-plugin/`** (v1.1+) bundles **weekly-brief** (orchestrator + LinkedIn/blog calendars), **reddit_quora_market_intelligence**, **google_trends_report**, and **setup**. Register via **`.claude-plugin/marketplace.json`**: `/plugin marketplace add <repo>` → `/plugin install living-strategy-plugin@living-strategy-agent` → `/reload-plugins`, or `claude --plugin-dir ./plugins/living-strategy-plugin`. See **`plugins/living-strategy-plugin/README.md`**.
...
3. **If the runtime is OpenClaw or otherwise has web/MCP tools** (search, fetch, browser), follow **`SKILL.md` → OpenClaw and tool-capable runtimes**: verify time-sensitive claims and refresh key URLs before finalizing the brief; never invent metrics; add **markdown links** `[label](https://...)` in **Strategic read** and **Trend signals** for grounded sources.
