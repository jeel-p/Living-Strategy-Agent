...
2. **Strategic read** — `## Strategic read`  
   Two to three sentences: interpretation for this brand now; underlying shift; signal for roughly the next four weeks. **Not** a summary of pasted inputs. When the insight depends on a specific page or article (user link or tool-grounded), include a **markdown link** `[label](https://...)`.
...
3. **Trend signals** — `## Trend signals (ranked by urgency)`  
   ...
   When the user supplied **`custom_sources`** or the runtime used **web/fetch tools**, tie signals to **named sources** and use **`[label](https://...)`** for supporting URLs (not raw bare links). If something is extrapolated beyond the provided list, mark it **`[INFERRED]`** (especially when `source_scope` is `provided_only`).
...
- **Specificity:** named platforms and markets throughout; **preserve user-provided source names** (URLs, `@handles`, `r/subreddit`, newsletter titles) when `custom_sources` is present; **render URLs as markdown links** where they support a claim.
