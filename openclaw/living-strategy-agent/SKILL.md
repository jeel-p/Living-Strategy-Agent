---
name: living_strategy_agent
description: Weekly marketing strategy briefs from trend signals and custom_sources. Use web search, URL fetch, or browser when available; include markdown source links in Strategic read and Trend signals. For living strategy, weekly briefs, competitive intelligence, and trend monitoring.
version: 1.0.0
---
...
4. **Linked sources (mandatory when URLs exist)** — Whenever a claim is grounded in **web search, fetch, or user-pasted links**, include at least one **clickable markdown link** per grounded signal where practical: `[short label](https://full-url)`. Put links in **Strategic read** (if the insight depends on a specific article or page) and in each **Trend signals** bullet that relies on external evidence. **User-provided** `custom_sources` URLs must appear as links, not plain text, when they support that bullet. Do **not** add a separate “Sources” or “References” top-level section; links stay **in-line** inside the mandatory sections only.
5. **Handles and platforms without a stable URL** — If only an `@handle` or platform name is known, name them explicitly; add a markdown link **only** when the tool or user supplied a canonical URL (do not guess broken links).
6. **Host policy** — Obey the user’s tool permissions, blocklists, and rate limits. If search or fetch is unavailable or blocked, state that once and proceed from **user-pasted** inputs only.
...
Use the **manual MVP** path: rely on pasted **trends**, **events**, **competitors**, **brand context**, and **`[INFERRED]`** labeling per **`source_scope`**—unchanged from offline use. If the user **pastes URLs** in inputs or `custom_sources`, still format those as **markdown links** in **Trend signals** / **Strategic read** where they support a claim.
...
- **Link sources** — When `custom_sources` includes URLs (or the user pasted links in trends/competitors), render them as **markdown links** `[label](url)` in **Trend signals** (and **Strategic read** if relevant), not bare strings.
...
## Strategic read
2-3 sentences. Interpretation: what the data means for this brand now; underlying shift; what this week signals for roughly the next four weeks. Not a restatement of inputs. When the read depends on a specific page or article, include a markdown link `[label](https://...)`.

## Trend signals (ranked by urgency)
For each signal:
- What it is (platform, market, metric if available); when `custom_sources` exists or web tools were used, include **markdown links** `[label](https://...)` to named URLs, official pages, or articles—not bare URLs. Tie handles/communities/newsletters by name; link when a canonical URL was provided or returned by tools.
...
- [ ] **Linked sources**: Grounded claims use **`[label](https://...)`** in **Strategic read** / **Trend signals** where URLs exist (tools or user); **custom_sources** URLs are linked, not left as raw text.
...
- Where the user supplied a URL, render it as a markdown link [label](https://...) in Trend signals (or Strategic read) when that source supports the point; choose a short readable label (brand, article title, or domain).
...
- For every trend signal or strategic point grounded in a fetched page or search result, include at least one markdown link [label](https://...) to the canonical page or article inside Strategic read or Trend signals—no separate References section.
- Optionally add the access or publication date in-line after the link when it strengthens the signal; keep wording tight.
...
about the next 4 weeks? If the interpretation rests on specific web evidence, include markdown links [label](https://...).

## Trend signals (ranked by urgency)
For each signal:
- State what it is precisely (platform, market, metric if available); when `custom_sources` was provided or tools returned URLs, tie the signal to named URLs, handles, communities, or newsletters and use markdown links [label](https://...) for any page or article that supports the signal
